...
def T(set1, set2, order, verbose=True, normalise=True, compute_sigs=True):
    ...
    X = Parallel(n_jobs=1)(delayed(Phi)(path, order, normalise, compute_sigs) for path in tqdm(set1, disable=(not verbose)))
    Y = Parallel(n_jobs=1)(delayed(Phi)(path, order, normalise, compute_sigs) for path in tqdm(set2, disable=(not verbose)))
