...
class Metrics_monitor:
    def __init__(self, machine, metrics = ['rec_error','sq_error'] , steps = None, n_chains=None):
    ...
    def QQ(self,res_int,  data, xmas, xmin):
        ...
        pp_x = sm.ProbPlot(data.iloc[:,0].T)
        pp_y = sm.ProbPlot(res_int.iloc[:,0].T)
        fig=qqplot_2samples(pp_x,pp_y, xlabel="Quantiles of Data", ylabel="Quantiles of RBM", line="45")
        fig.suptitle("RBM vs. Data "+ticker[0:6])
    ...
    def fit(self,test_set,qqraw_data, xmas, xmin,n_points = None):
        ...
        samples, prob, _ = self.machine.parallel_sample([],n_step_MC=self.steps)
        ...
        for i in list(qqraw_data.columns):
            q=self.QQ(res_int,  qqraw_data, xmas, xmin)
            n='QQ_Plot '+i
